...
if ($w == '' || $w == 'u') {
    ...
    if ($w=='') {
		...
		// LottoAI: 커스텀 회원가입 폼에서는 ajax 중복확인 세션을 사용하지 않으므로
		// 세션값만 초기화하고 경고는 띄우지 않는다.
		if (get_session('ss_check_mb_id') != $mb_id
			|| get_session('ss_check_mb_nick') != $mb_nick
			|| get_session('ss_check_mb_email') != $mb_email) {
			set_session('ss_check_mb_id', '');
			set_session('ss_check_mb_nick', '');
			set_session('ss_check_mb_email', '');
			// alert 생략 → 아래에서 exist_mb_id/nick/email 로 중복 여부 다시 검사
		}
