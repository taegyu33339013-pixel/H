...
const lottoGenerator = {
  ...
  analyzeStats() {
    ...
    // 평균 출현 주기 계산
    const lastSeenValues = Object.values(lastSeen);
    const avgGap = lastSeenValues.reduce((sum, v) => sum + v, 0) / lastSeenValues.length;
    ...
    while (attempts < maxAttempts) {
      switch(style) {
        ...
        case 'color':
          ...
          break;
        case 'montecarlo':
          numbers = this._generateMonteCarlo(stats);
          break;
        case 'sum':
          numbers = this._generateSumOptimized(stats);
          break;
        case 'cycle':
          numbers = this._generateCycleBased(stats);
          break;
        case 'lastdigit':
          numbers = this._generateLastDigitBalanced(stats);
          break;
        case 'consecutive':
          numbers = this._generateConsecutiveAware(stats);
          break;
        case 'balanced':
        default:
  ...
  _generateMonteCarlo(stats) {
    ...
    for (let i = 0; i < simulations; i++) {
      ...
      candidateSets.push({ numbers: sorted, score });
    }
    
    // 점수 상위 10%에서 랜덤 선택
    candidateSets.sort((a, b) => b.score - a.score);
    const topTen = candidateSets.slice(0, Math.ceil(simulations * 0.1));
    const selected = topTen[Math.floor(Math.random() * topTen.length)];
    
    return selected.numbers;
  },

  // 합계 최적화 생성 (100~170 구간)
  _generateSumOptimized(stats) {
    const targetMin = 100;
    const targetMax = 170;
    const targetMid = 135;
    let bestNumbers = [];
    ...
    for (let attempt = 0; attempt < 200; attempt++) {
      ...
      const sum = numbers.reduce((a, b) => a + b, 0);
      const diff = Math.abs(sum - targetMid);
      
      if (sum >= targetMin && sum <= targetMax && diff < bestDiff) {
        bestDiff = diff;
        bestNumbers = [...numbers];
        if (diff < 10) break;
      }
  ...
  // 주기 분석 기반 생성
  _generateCycleBased(stats) {
    const numbers = [];
    const avgGap = stats.avgGap || 7;
    
    // 출현 주기가 평균에 가까운 번호들 우선 선택
    const cycleScored = [];
    for (let n = 1; n <= 45; n++) {
      const lastSeen = stats.lastSeen[n] || 0;
      const cycleDiff = Math.abs(lastSeen - avgGap);
      cycleScored.push({ num: n, score: 10 - cycleDiff });
    }
    cycleScored.sort((a, b) => b.score - a.score);
    
    // 상위 15개에서 6개 랜덤 선택
    const pool = cycleScored.slice(0, 15);
    while (numbers.length < 6 && pool.length > 0) {
      const idx = Math.floor(Math.random() * pool.length);
      numbers.push(pool.splice(idx, 1)[0].num);
    }
  ...
  // 끝수 균형 생성 (0~9 끝자리 분산)
  _generateLastDigitBalanced(stats) {
    const numbers = [];
    const usedLastDigits = new Set();
    let attempts = 0;
    ...
    while (numbers.length < 6 && attempts < 100) {
      ...
      // 끝자리 중복 최소화 (최대 2개까지만 같은 끝수 허용)
      const sameDigitCount = numbers.filter(num => num % 10 === lastDigit).length;
      
      if (!numbers.includes(n) && sameDigitCount < 2) {
        numbers.push(n);
        usedLastDigits.add(lastDigit);
      }
      attempts++;
    }
  ...
  // 연속번호 패턴 분석 기반 생성
  _generateConsecutiveAware(stats) {
    const numbers = [];
    const includeConsecutive = Math.random() < 0.65; // 65% 확률로 연속번호 1쌍 포함
    
    if (includeConsecutive) {
      // 연속번호 1쌍 생성
      const start = Math.floor(Math.random() * 44) + 1;
      numbers.push(start, start + 1);
    }
    ...
    while (numbers.length < 6) {
      const n = Math.floor(Math.random() * 45) + 1;
      if (!numbers.includes(n)) {
        // 이미 연번 쌍이 있으면 추가 연번 방지
        const hasAdjacentInNumbers = numbers.some(existing => 
          Math.abs(existing - n) === 1
        );
        if (includeConsecutive && numbers.length >= 2 && hasAdjacentInNumbers) {
          continue; // 추가 연번 방지
        }
        numbers.push(n);
      }
