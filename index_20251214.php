...
<head>
  ...
  <!-- Primary Meta Tags -->
  <title>로또인사이트 - AI 기반 로또 번호 분석 | <?= (int)$max_round ?>회차 데이터 실시간 반영</title>
  <meta name="title" content="로또인사이트 - AI 기반 로또 번호 분석 | <?= (int)$max_round ?>회차 데이터 실시간 반영">
  ...
  <meta property="og:title" content="로또인사이트 - AI 기반 로또 번호 분석">
  ...
  <meta property="twitter:title" content="로또인사이트 - AI 기반 로또 번호 분석">
...
<body>
  ...
  <div class="loading-screen" id="loadingScreen">
    ...
    <div class="loading-text">로또인사이트</div>
      ...
      <div class="archive-card">
        ...
        <div class="archive-verify">
          ...
          <div class="archive-verify-input-group">
            <input type="number" class="archive-verify-input" id="verifyInput" placeholder="회차 입력 (예: <?php echo $round;?>)" min="1" max="1201">
  ...
  <!-- Quality Gauge Section -->
  <?php
	function lotto_balance_score(array $nums): int {
	  sort($nums);
	  $odd = 0; $low = 0;
	  $color = [0,0,0,0,0]; // 1~10, 11~20, 21~30, 31~40, 41~45
	  ...
	  // 홀짝/고저는 3에서 멀어질수록 감점
	  $odd_pen = abs(3 - $odd) * 12;   // 0,12,24,36
	  $low_pen = abs(3 - $low) * 12;

	  // 색상 몰림(최대 그룹 카운트가 3 넘어가면 추가 감점)
	  $max_color = max($color);
	  $color_pen = max(0, ($max_color - 2)) * 10; // 3개면 10, 4개면 20...

	  $score = 100 - ($odd_pen + $low_pen + $color_pen);
	  return max(0, min(100, $score));
	}

	function lotto_ac_value(array $nums): float {
	  sort($nums);
	  $diffs = [];
	  $len = count($nums);
	  for ($i=0; $i<$len; $i++) {
	  ...
	  $distinct = count($diffs);
	  $ac = $distinct - ($len - 1); // 6개면 -5
	  if ($ac < 0) $ac = 0;
	  if ($ac > 10) $ac = 10;
	  return round($ac, 1);
	}

	// ✅ MVP 임시 구현: 패턴 점수(0~100)
	// - 6개 번호의 간격/연속성/중복구간(없어야 정상) 등을 단순 평가
	if (!function_exists('lotto_pattern_score')) {
	  function lotto_pattern_score(array $nums): int {
		sort($nums);
		$len = count($nums);
		if ($len !== 6) return 0;

		// 연속(예: 12,13 같이 붙는 쌍) 개수
		$consecutive = 0;
		for ($i=0; $i<$len-1; $i++) {
		  if (($nums[$i+1] - $nums[$i]) === 1) $consecutive++;
		}

		// 스프레드(최대-최소)가 너무 좁으면 감점 (너무 몰림)
		$spread = $nums[$len-1] - $nums[0]; // 0~44
		$spread_pen = 0;
		if ($spread < 18) $spread_pen = 25;       // 너무 좁음
		elseif ($spread < 22) $spread_pen = 12;   // 약간 좁음

		// 연속이 많을수록 감점 (0~5)
		$cons_pen = $consecutive * 10; // 1쌍=10, 2쌍=20...

		$score = 100 - ($spread_pen + $cons_pen);
		return max(0, min(100, (int)$score));
	  }
	}

	// ✅ MVP 임시 구현: Hot/Cold 점수(0~100)
	// - 최근 N회(기본 20회)에서 각 번호 출현 빈도로 Hot/Cold 계산
	// - 너무 "핫"으로 쏠리거나, 너무 "콜드"로 쏠리면 감점
	if (!function_exists('lotto_hotcold_score')) {
	  function lotto_hotcold_score(array $nums, int $recent = 20): int {
		global $g5;

		sort($nums);
		if (count($nums) !== 6) return 0;

		$recent = max(5, min(200, $recent));

		// 최근 N회 당첨번호 로드 (테이블/컬럼은 운영 DB에 맞게 수정!)
		// 아래는 예시: g5_lotto_draw(draw_no, n1~n6)
		$rows = sql_query("
		...
		// 선택번호의 평균 출현횟수
		$sum = 0;
		foreach ($nums as $n) $sum += $freq[$n] ?? 0;
		$avg = $sum / 6.0;

		// 최근 N회에서 기대 평균은 대략 (6/45)*N = N/7.5
		$expected = $recent / 7.5;

		// 너무 핫/콜드로 치우치면 감점: expected와의 차이가 클수록 감점
		$diff = abs($avg - $expected);

		// 스케일링: diff가 0이면 100점, diff가 커질수록 감점
		// 대충 diff 3이면 70점, diff 5면 50점 정도로 떨어지게
		$pen = (int)round($diff * 10); // diff 3 => 30 감점
		$score = 100 - $pen;
	...
	$balance = lotto_balance_score($nums);
	$ac      = lotto_ac_value($nums);

	// pattern_score, hotcold_score는 MVP로 일단 0~100 계산 함수 붙이면 됩니다.
	$pattern = lotto_pattern_score($nums);   // (추가 구현)
	$hotcold = lotto_hotcold_score($nums);   // (추가 구현)

	$nums_str = implode(',', $nums);
	sql_query("
	  INSERT INTO g5_lotto_ai_pick_log
	  (mb_id, nums, balance_score, ac_value, pattern_score, hotcold_score, created_at)
	  VALUES
	  ('".sql_real_escape_string($member['mb_id'] ?? '')."', '".sql_real_escape_string($nums_str)."',
	   {$balance}, {$ac}, {$pattern}, {$hotcold}, NOW())
	");
	...
	$balance = (float)($avg['balance'] ?? 0);
	$ac      = (float)($avg['ac'] ?? 0);
	$pattern = (float)($avg['pattern'] ?? 0);
	$hotcold = (float)($avg['hotcold'] ?? 0);

	$ac_pct = max(0, min(100, $ac * 10)); // AC 0~10 -> 0~100
  ?>
    ...
    <div class="quality-container">
      <div class="quality-card">
        <div class="quality-header">
          ...
          <p class="quality-subtitle">로또인사이트 AI가 생성하는 번호의 평균 품질 지표</p>
        </div>
        <div class="quality-gauges">
          ...
          <div class="quality-gauge">
            <div class="quality-gauge-header">
              ...
              <span class="quality-gauge-value"><?php echo number_format($balance, 1); ?>점</span>
            </div>
            <div class="quality-bar">
          ...
          <div class="quality-gauge">
            ...
            <div class="quality-bar">
			  <div class="quality-fill good" style="width: <?php echo (int)$ac_pct; ?>%"></div>
            </div>
      ...
      <div class="algorithm-card">
        <div class="algorithm-steps">
          <div class="algorithm-step">
            ...
            <p class="algorithm-step-desc">동행복권 공식 API에서<br>1~<?= (int)$max_round ?>회차 데이터 수집</p>
          </div>
              ...
              <?php foreach ($hot_numbers as $item): ?>
                <?php
                  ...
                  $cls   = function_exists('lotto_ball_color_class')
                            ...
                            : 'ball-yellow';
                ?>
                <div class="hot-ball <?php echo $cls; ?>">
                  <?php echo $num; ?>
              ...
              <?php foreach ($cold_numbers as $item): ?>
                <?php
                  ...
                  $cls   = function_exists('lotto_ball_color_class')
                            ...
                            : 'ball-yellow';
                ?>
                <div class="cold-ball <?php echo $cls; ?>">
                  <?php echo $num; ?>
                  <span class="ball-count"><?php echo number_format($count); ?></span>
                </div>
              <?php endforeach; ?>
...
<section class="warning-section">
    ...
    <div class="warning-card">
      ...
      <div class="warning-footer">
        ...
        <p class="warning-footer-text">
          로또인사이트 AI는 위 패턴을 <strong>룰 기반 필터로 자동 회피</strong>하여
          더 균형 잡힌 조합만 생성합니다.
        </p>
      </div>
    </div>
      ...
      <div class="pricing-grid" style="grid-template-columns: repeat(3, 1fr);">
        <div class="pricing-card">
          ...
          <ul class="pricing-features">
            ...
            <li><span class="check">✓</span> 5가지 분석 스타일</li>
    ...
    <div class="faq-container">
      ...
      <div class="faq-list">
        ...
        <div class="faq-item">
          ...
          <div class="faq-answer">
            <p class="faq-answer-content">
              아니요. 로또인사이트는 만 19세 이상만 이용할 수 있습니다. 
  ...
  <div class="share-modal" id="shareModal">
    <div class="share-modal-content">
      ...
      <div class="share-modal-preview">
        ...
        <p class="share-modal-text">로또인사이트 AI가 분석한 균형 잡힌 번호!</p>
      </div>
    ...
    <div class="footer-container">
      ...
      <div class="footer-bottom">
        <p class="footer-copyright">
          © 2025 로또인사이트. All rights reserved.
        </p>
      ...
      <div class="footer-company">
        <div class="footer-company-info">
          <div class="footer-company-item">상호:<span>로또인사이트</span></div>
  ...
  <script>
    ...
    const styles = ['Hot/Cold 분석', '밸런스 최적화', '홀짝/고저 분석', 'AI 추천', 'AC값 분석', '패턴 분석'];
      ...
      if (typeof Kakao !== 'undefined' && Kakao.isInitialized()) {
        ...
        Kakao.Share.sendDefault({
          ...
          content: {
            ...
            description: '로또인사이트 - <?= (int)$round ?>회차 데이터 기반 AI 분석\n무료 1회 분석 즉시 제공!',
